...
%
% Trying MLE
Samples_d = double(Samples);
mu = mean(Samples_d);
difference = Samples_d - ones(length(Samples_d),1)*mu;
dummy = zeros(length(mu),length(mu));
for ct = 1:length(Samples_d)
    dummy = dummy + difference(ct,:)'*difference(ct,:);
end
covar = dummy/length(Samples_d);
